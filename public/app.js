...
const screens = {
  ...
  complaint: document.querySelector("#chatScreen"),
...
const titles = {
  home: "Resolve legal issues in minutes",
  complaint: "Consumer complaint draft",
...
async function generateDraft() {
  addMessage("bot", "Done. I am preparing a simple complaint draft now.");
  ...
  renderDraftPreview(state.latestComplaint, complaintPreview);
  state.latestCaseId = saveComplaintCase("Drafted");
  showScreen("output");
}
...
answerForm.addEventListener("submit", async (event) => {
  ...
  addMessage("user", answerSummary(question, value));
  state.index += 1;
...
document.querySelector("#markSubmittedButton").addEventListener("click", () => {
  updateCaseStatus(state.latestCaseId, "Submitted");
...
document.querySelector("#escalateButton").addEventListener("click", () => {
  ...
  updateCaseStatus(state.latestCaseId, "Escalated");
  escalationText.scrollIntoView({ behavior: "smooth", block: "nearest" });
});
...
rtiForm.addEventListener("submit", (event) => {
  ...
  document.querySelector("#rtiOutput").scrollIntoView({ behavior: "smooth", block: "start" });
});
...
function saveComplaintCase(status) {
...
function updateCaseStatus(id, status) {
  ...
  const cases = loadCases();
  const nextCases = cases.map((item) => (item.id === id ? { ...item, status } : item));
  saveCases(nextCases);
}
...
function renderCases() {
  ...
  cases.forEach((item) => {
    ...
    ["Drafted", "Submitted", "Escalated"].forEach((status) => {
    ...
    select.addEventListener("change", () => {
      updateCaseStatus(item.id, select.value);
